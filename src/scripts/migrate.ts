...
async function loadJsonData(fileName: string): Promise<TWordData[]> {
    const fullPath = path.join(__dirname, "../mock_data", fileName);
...
async function migrate() {
    try {
        ...
        for (const group of wordGroups) {
            ...
            const translations = group.filter((w) => w.language !== baseWordData.language);
            ...
            console.log(`✅ Migrated group: ${baseWordData.wordId} (${baseWordData.text})`);
        }
